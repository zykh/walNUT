...
// Do: exec argv and returns an array of stdout and stderr, if any (otherwise it returns null, e.g.: [stdout, null] -> no errors)
...
function getUps(timeoutPath, timeout, notify, upsc, host, port) {
	...
	// If list is void we'll check localhost:3493
	if (!host && l == 0)
		host = 'localhost';
	if (!port && l == 0)
		port = '3493';
	...
	// Return stored UPS + availability
...
function upsCred(user, pw) {
	...
	// If user or password are void we'll delete them from UPS properties
	if (user.length > 0)
		got[0].user = user;
	else
		delete got[0].user;
