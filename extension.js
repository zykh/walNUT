...
// Max length (in chars)
//	ErrorBox Label
const ERR_LABEL_LENGTH = 35;
//	ErrorBox Description
const ERR_DESC_LENGTH = 40;
//	device manufacturer+model
const MODEL_LENGTH = 40;
//	topdata (status/alarm) description (2nd row)
const TOPDATA_LENGTH = 40;
//	raw data list: variable name
const RAW_VAR_LENGTH = 35;
//	raw data list: variable value
const RAW_VALUE_LENGTH = 40;
//	UPS commands list - description @ submenu
const CMD_LENGTH = 45;
// 	UPS commands list - description @ combobox
const CMD_DESC_LENGTH = 45;
//	Credentials dialog description
const CRED_DIALOG_LENGTH = 60;

// Interval in milliseconds after wich the menu, if open, has to be updated (15 minutes)
...
const UpscMonitor = new Lang.Class({
	...
	_init: function() {
		...
		// Here we'll store chosen UPS variables
		this._ups = {};
		this._vars = Array();
...
const walNUT = new Lang.Class({
	...
	updatePanelIcon: function() {
		...
		// UPS isn't telling us it's charging or discharging -> we suppose it's charged
		} else
	...
	refreshMenu: function(hasChanged) {
		...
		if (this._state == ErrorType.NO_ERROR) {
			...
			// Don't show table if no datas is available
			if (count)
...
const CredDialog = new Lang.Class({
	...
	// _updateOkButton: The exec button will be reactive only if both username and password are set (length > 0)
	...
	// _onOk: actions to do when Exec button is pressed
	_onOk: function() {
...
const CredBox = new Lang.Class({
	...
	// Update credentials: if void user or password is given it'll be removed from UPS properties
	credUpdate: function() {
...
const UpsCmdList = new Lang.Class({
	...
	// retrieveCmds: get instant commands from UPS through upscmd and return success/failure status
	...
	// cmdExec: try and exec the command cmd
	cmdExec: function(user, pw, cmd) {
		...
		if (user && pw) {
			...
			// stderr = OK\n -> Command sent to driver successfully
...
// UpsRawDataList: listing UPS raw data in a submenu
const UpsRawDataList = new Lang.Class({
	...
	buildInfo: function() {
		...
		let actual;
		// Object where keys are variable names (battery.charge, ups.status..) that store the original position of children (used to delete vars no longer available)
		let stored = {};
		// Array of submenu children's variable names (used to sort new vars alphabetically)
		let ab = Array();
		...
		for each (let item in this._vars){
			...
			// Submenu doesn't have children or the current var isn't one of them
				...
				// If the var is new as the submenu add it at the default position, as vars are already alphabetically ordered
				else
...
const UpsDataTable = new Lang.Class({
	...
	_init: function(i, l, t) {
		...
		// New table
		this.table = new St.Table({ style_class: 'waln' });
...
// UpsModel: Listing chosen UPS Model/manufacturer (if available)
...
// UpsList: Listing Available UPSes in combobox in upsc-like way (e.g. ups@hostname:port)
const UpsList = new Lang.Class({
	...
	// update: Void combobox and update it with the new device list
...
const NutMenu = new Lang.Class({
	...
	_init: function(sourceActor) {
		...
		// Separator between chosen UPS data & raw data/UPS commands
...
// Panel/menu
let walnut;
// Absolute path of upsc
let upsc;
// Absolute path of upscmd
let upscmd;
// Absolute path of timeout
let timeout;
...
function parseStatus(raw, icon) {
	...
	for (let i = 0; i < st.length; i++) {
		switch(st[i]){
			...
			case 'OL': line += _(" on line");	icon_line = 'o';	break;
			// TRANSLATORS: Device status @ device status box
			case 'OB': line += _(" on battery");	icon_line = 'b';	break;
			// TRANSLATORS: Device status @ device status box
			case 'LB': status += _(", low battery");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'RB': status += _(", replace battery");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'CHRG': status += _(", charging");	break;
			// TRANSLATORS: Device status @ device status box
			case 'DISCHRG': status += _(", discharging");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'BYPASS': status += _(", bypass");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'CAL': status += _(", runtime calibration");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'OFF': status += _(", offline");	break;
			// TRANSLATORS: Device status @ device status box
			case 'OVER': status += _(", overloaded");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'TRIM': status += _(", trimming");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'BOOST': status += _(", boosting");	icon_alarm = 'a';	break;
			// TRANSLATORS: Device status @ device status box
			case 'FSD': status += _(", forced shutdown");	icon_alarm = 'a';	break;
			case 'ALARM': icon_alarm = 'a';	break;
			default: break;
		}
	}
...
function cmdI18n(cmd) {
	switch(cmd['cmd']){
		...
		case 'load.off': cmd['desc'] = _("Turn off the load immediately");	break;
		// TRANSLATORS: UPS Command description
		case 'load.on': cmd['desc'] = _("Turn on the load immediately");	break;
		// TRANSLATORS: UPS Command description
		case 'load.off.delay': cmd['desc'] = _("Turn off the load possibly after a delay");	break;
		// TRANSLATORS: UPS Command description
		case 'load.on.delay': cmd['desc'] = _("Turn on the load possibly after a delay");	break;
		// TRANSLATORS: UPS Command description
		case 'shutdown.return': cmd['desc'] = _("Turn off the load possibly after a delay and return when power is back");	break;
		// TRANSLATORS: UPS Command description
		case 'shutdown.stayoff': cmd['desc'] = _("Turn off the load possibly after a delay and remain off even if power returns");	break;
		// TRANSLATORS: UPS Command description
		case 'shutdown.stop': cmd['desc'] = _("Stop a shutdown in progress");	break;
		// TRANSLATORS: UPS Command description
		case 'shutdown.reboot': cmd['desc'] = _("Shut down the load briefly while rebooting the UPS");	break;
		// TRANSLATORS: UPS Command description
		case 'shutdown.reboot.graceful': cmd['desc'] = _("After a delay, shut down the load briefly while rebooting the UPS");	break;
		// TRANSLATORS: UPS Command description
		case 'test.panel.start': cmd['desc'] = _("Start testing the UPS panel");	break;
		// TRANSLATORS: UPS Command description
		case 'test.panel.stop': cmd['desc'] = _("Stop a UPS panel test");	break;
		// TRANSLATORS: UPS Command description
		case 'test.failure.start': cmd['desc'] = _("Start a simulated power failure");	break;
		// TRANSLATORS: UPS Command description
		case 'test.failure.stop': cmd['desc'] = _("Stop simulating a power failure");	break;
		// TRANSLATORS: UPS Command description
		case 'test.battery.start': cmd['desc'] = _("Start a battery test");	break;
		// TRANSLATORS: UPS Command description
		case 'test.battery.start.quick': cmd['desc'] = _("Start a \"quick\" battery test");	break;
		// TRANSLATORS: UPS Command description
		case 'test.battery.start.deep': cmd['desc'] = _("Start a \"deep\" battery test");	break;
		// TRANSLATORS: UPS Command description
		case 'test.battery.stop': cmd['desc'] = _("Stop the battery test");	break;
		// TRANSLATORS: UPS Command description
		case 'calibrate.start': cmd['desc'] = _("Start runtime calibration");	break;
		// TRANSLATORS: UPS Command description
		case 'calibrate.stop': cmd['desc'] = _("Stop runtime calibration");	break;
		// TRANSLATORS: UPS Command description
		case 'bypass.start': cmd['desc'] = _("Put the UPS in bypass mode");	break;
		// TRANSLATORS: UPS Command description
		case 'bypass.stop': cmd['desc'] = _("Take the UPS out of bypass mode");	break;
		// TRANSLATORS: UPS Command description
		case 'reset.input.minmax': cmd['desc'] = _("Reset minimum and maximum input voltage status");	break;
		// TRANSLATORS: UPS Command description
		case 'reset.watchdog': cmd['desc'] = _("Reset watchdog timer (forced reboot of load)");	break;
		// TRANSLATORS: UPS Command description
		case 'beeper.enable': cmd['desc'] = _("Enable UPS beeper/buzzer");	break;
		// TRANSLATORS: UPS Command description
		case 'beeper.disable': cmd['desc'] = _("Disable UPS beeper/buzzer");	break;
		// TRANSLATORS: UPS Command description
		case 'beeper.mute': cmd['desc'] = _("Temporarily mute UPS beeper/buzzer");	break;
		// TRANSLATORS: UPS Command description
		case 'beeper.toggle': cmd['desc'] = _("Toggle UPS beeper/buzzer");	break;
		default: // outlet.n.{shutdown.return,load.off,load.on,load.cycle}
			if (cmd['cmd'].slice(0,6) == 'outlet') {
				let buf = cmd['cmd'].split('.');
				switch(buf[2]+'.'+buf[3]){
					// TRANSLATORS: UPS Command description
					case 'shutdown.return': cmd['desc'] = _("Turn off the outlet #%d possibly after a delay and return when power is back").format(buf[1]);	break;
					// TRANSLATORS: UPS Command description
					case 'load.off': cmd['desc'] = _("Turn off the outlet #%d immediately").format(buf[1]);	break;
					// TRANSLATORS: UPS Command description
					case 'load.on': cmd['desc'] = _("Turn on the outlet #%d immediately").format(buf[1]);	break;
					// TRANSLATORS: UPS Command description
					case 'load.cycle': cmd['desc'] = _("Power cycle the outlet #%d immediately").format(buf[1]);	break;
					default: break;
				}
			} else break;
	}
